	...
	public class CurrentQueueViewController : BaseViewController
	{
		...
		public void ScrollToCurrentSong()
		{
			...
			try {
				...
				if (path.Section < section && path.Row <rows )
		...
		public class Source : UITableViewSource
		{
			...
			{
				if (!Settings.Random)
					return Util.SongsDict [Util.PlayListSorted [indexPath.Row]];
				else
				try {
					switch (indexPath.Section) {
					case 0:
						return indexPath.Row == Util.PlayListPlayed.Count ? Util.CurrentSong : Util.SongsDict [Util.PlayListPlayed [indexPath.Row]]; //Util.SongsDict[Util.PlayListSorted[indexPath.Row]];//.Where(x=> x.IndexCharacter == headers[indexPath.Section]).ToArray()[indexPath.Row];
					case 1:
						return indexPath.Row == 0 && Util.NextSong != null ? Util.NextSong : Util.SongsDict [Util.NextSongs [indexPath.Row - (Util.NextSong == null ? 0 : 1)]];
					case 2:
						return Util.SongsDict [Util.PlayList [indexPath.Row]];
					}
				} catch (Exception ex) {
					return new Song ();
				}
