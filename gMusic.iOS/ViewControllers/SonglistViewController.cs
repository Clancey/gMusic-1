...
{
	public class SongListViewController : BaseViewController, IBaseViewController
	{
		private SongViewModel DataSource;
		bool IsTvOut;
		string Filter;
		string OrderBy;
		public SongListViewController(string title,string filter, string orderby) : this(true) 
		{
			Title = title;
			Filter = filter;
			OrderBy = orderby;
		...
		{
			base.setupTable ();			
			DataSource = new SongViewModel(this,Filter,OrderBy);
